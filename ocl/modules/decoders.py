...
from ocl.modules.utils import WNConv3d, WNConv
from ocl.modules import networks, utils
...
def build(config, name: str, **kwargs):
    if name == "SpatialBroadcastDecoder":
        ...
        # proj_slots = None
        # if config.get("proj_slots"):
        #     proj_slots = utils.build_module(config.proj_slots)
        output_transform = None
        if config.get("output_transform"):
            output_transform = utils.build_module(config.output_transform)
...
class SlotMixerDecoder(nn.Module):
    ...
    def __init__(
        ...
        allocator: nn.Module,
        renderer: nn.Module,
        train_chunk: Optional[int] = None,
        val_chunk: Optional[int] = None,
        ...
        eval_output_size: Optional[Tuple[int]] = None,
        proj_slots: Optional[Dict] = None,
        final_conv: Optional[str] = 'wnconv3d',
        kernel_size: Optional[int] = None,
    ):
        super().__init__()
        ...
        if final_conv == 'wnconv3d':
            self.final_conv = WNConv3d(
                in_channels=1,
                out_channels=1,
                kernel_size=kernel_size,
                padding=kernel_size // 2,
        ...
        self.val_chunk = val_chunk
        self.proj_slots = proj_slots
        if proj_slots["name"] == "networks.two_layer_mlp":
            self.proj = True
            # self.norm_slots = nn.LayerNorm(proj_slots["inp_dim"] * chunk, eps=1e-5)
            ...
            # self.out_dim = proj_slots["outp_dim"]
            # self.proj = proj_slots
            # self.proj = nn.Sequential(
            #     nn.Linear(inp_dim * 2, inp_dim * 4),
            #     nn.ReLU(inplace=True),
            #     nn.Linear(inp_dim * 4, inp_dim)
            # )
            self.normalization = nn.LayerNorm(proj_slots["inp_dim"])
            # self.dropout = nn.Dropout1d(p=0.1)
            # self.proj = True
            # self.cluster1 = nn.Sequential(
            #     nn.Linear(proj_slots["inp_dim"], proj_slots["outp_dim"]),
            #     # nn.Dropout(p=proj_slots["dropout"])
            # )
            ...
            #     nn.Dropout(p=proj_slots["dropout"])
            # )
        elif proj_slots["name"] == "linear":
            self.normalization = nn.LayerNorm(proj_slots["inp_dim"])
            self.proj = nn.Linear(proj_slots["inp_dim"], proj_slots["outp_dim"])
    ...
    def forward(self, slots: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        if self.proj is not None and self.proj_slots["name"] == "networks.two_layer_mlp":
            norm_slots = self.normalization(slots)
            slots = self.proj(norm_slots)
            # slots = slots.unflatten(0, (slots.shape[0] // self.chunk, self.chunk)).permute(0, 2, 1, 3).flatten(-2, -1)
            # slots = self.proj(self.norm_slots(slots))
            # slots = slots.unflatten(-1, (self.chunk, self.out_dim)).permute(0, 2, 1, 3).flatten(0, 1)
            # norm_slots = self.norm_slots(slots)
            # with torch.no_grad():
            #     slots1 = self.cluster1(norm_slots)
            # slots = slots1 + self.cluster2(norm_slots)
        elif self.proj is not None and self.proj_slots["name"] == "linear":
            slots = self.proj(slots)
            slots = self.normalization(slots)
        ...
        if self.renderer is not None:
            ...
            features = mixed_slots
        recons = self.output_transform(features)
        masks = attn.transpose(-2, -1)
        conv_masks = None
        if isinstance(self.final_conv, WNConv3d):
            if self.training:
                unflattened_masks = masks.unflatten(0, (masks.shape[0] // self.train_chunk, self.train_chunk))
            else:
                unflattened_masks = masks.unflatten(0, (masks.shape[0] // self.val_chunk, self.val_chunk))
            b, t, k, d = unflattened_masks.shape
            conv_masks = self.final_conv(unflattened_masks.permute(0, 2, 1, 3).unsqueeze(2).reshape(b*k, 1, t, int(d**0.5), int(d**0.5)))
            conv_masks = conv_masks.squeeze().reshape(b, k, t, d).permute(0, 2, 1, 3).flatten(0, 1)
        elif isinstance(self.final_conv, WNConv):
            b, k, d = masks.shape
            conv_masks = self.final_conv(masks.unsqueeze(2).reshape(b*k, 1, int(d**0.5), int(d**0.5)))
            conv_masks = conv_masks.squeeze().reshape(b, k, d)
