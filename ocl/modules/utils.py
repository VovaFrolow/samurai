...
from ocl.utils import config_as_kwargs, get_class_by_name, make_build_fn
...
def build_module(
    ...
    from ocl.modules import BUILD_FNS_BY_MODULE_GROUP
...
class WNConv3d(nn.Conv3d):
    """
    Сверточный 3D слой с нормализацией ядра (Weight Normalized Conv3d).
    
    Conv3d предназначен для обработки временных последовательностей изображений 
    (3D данных формата [batch, channels, time, height, width]).
    
    Аргументы:
        in_channels (int): Количество входных каналов
        out_channels (int): Количество выходных каналов
        kernel_size (int или tuple): Размер ядра свертки
        stride (int или tuple, опционально): Шаг свертки
        padding (int или tuple, опционально): Отступы
        bias (bool, опционально): Добавлять ли смещение (bias)
        norm_type (str, опционально): Тип нормализации - 'softmax' или 'linear'
        use_normed_logits (bool, опционально): Использовать ли нормализованные логиты
    """
    def __init__(
        self, 
        in_channels: int, 
        ...
        # Если kernel_size передан как целое число, преобразуем в кортеж
        if isinstance(kernel_size, int):
            kernel_size = (kernel_size, kernel_size, kernel_size)
        
        # Если padding передан как целое число, преобразуем в кортеж
        if isinstance(padding, int):
            padding = (padding, padding, padding)
            
        # Если stride передан как целое число, преобразуем в кортеж
        if isinstance(stride, int):
            stride = (stride, stride, stride)
            
        super(WNConv3d, self).__init__(
            in_channels, out_channels, kernel_size, 
            stride=stride, padding=padding, bias=bias
        )
        
        self.norm_type = norm_type
        self.use_normed_logits = use_normed_logits

    def forward(self, input):
        # Нормализация весов перед применением свертки
        o_c, i_c, k1, k2, k3 = self.weight.shape
        weight = torch.reshape(self.weight, (o_c, i_c, k1 * k2 * k3))
        
        # Нормализуем логиты до единичной нормы
        # if self.use_normed_logits:
        weight = weight / (torch.linalg.norm(weight, dim=-1, keepdim=True) + 1e-8)
        
        # Применяем выбранный тип нормализации
        if 'linear' in self.norm_type:
            weight = weight / (torch.sum(weight, dim=-1, keepdim=True) + 1e-8)
        elif 'softmax' in self.norm_type:
            weight = F.softmax(weight, dim=-1)
        
        # Преобразуем обратно в формат 5D тензора для Conv3d
        weight = torch.reshape(weight, (o_c, i_c, k1, k2, k3))
        
        # Обновляем веса слоя
        self.weight = nn.Parameter(weight)
        
        # Версия PyTorch 1.7+ требует другого вызова
        if '1.7' in torch.__version__:
            return self._conv_forward(input, self.weight)
        else:
            return self._conv_forward(input, self.weight, self.bias)